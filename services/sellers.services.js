...
async function sellerRegistration(params, callback) {
    console.log(params);
    const { seller_name, seller_phone, seller_address, seller_upi, image, community, delivery_type, membership_duration, sellerFssai } = params;

    if (!seller_name || !seller_phone || !seller_address || !seller_upi || !community || !delivery_type || !membership_duration || !sellerFssai) {
    ...
    try {
        ...
        const insertSellerQuery = 'INSERT INTO SELLER (seller_name, seller_phone, seller_address, seller_upi, seller_photo, seller_rating, seller_no_of_rating, community, delivery_type, membership_end_date, fssai_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
        await db.promise().query(insertSellerQuery, [seller_name, seller_phone, seller_address, seller_upi, image, 0.0, 0, community, delivery_type, membershipEndDate, sellerFssai]);
